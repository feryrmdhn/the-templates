...
var FALLBACK_DATA = FALLBACK['welcoming-baby-2'];
var _projectId = null;
...
function showNotification(message, type) {
  ...
  n.style.cssText = 'position:fixed;top:20px;left:50%;transform:translateX(-50%);z-index:99999;'
    + 'background:' + (type === 'success' ? 'linear-gradient(135deg, #7DD4B0, #F08080)' : '#E85D5D') + ';color:white;'
...
function initFloatingAnimations() {
  ...
  anime({
    targets: '.opening-content .floating-balloon',
    translateY: [-10, -25],
    rotate: [-8, 8],
    duration: 3000,
    direction: 'alternate',
...
function confettiBurst() {
  ...
  var colors = ['#7DD4B0', '#F08080', '#9BE0C4', '#D4C5F9', '#D4A96A'];
    ...
    btnOpen.addEventListener('click', function () {
      ...
      if (audio) {
        if (!audio.src || audio.src === window.location.href) {
          var musicSrc = (FALLBACK_DATA && FALLBACK_DATA.music) ? FALLBACK_DATA.music : '';
          if (musicSrc) {
            audio.src = musicSrc;
            audio.load();
          }
