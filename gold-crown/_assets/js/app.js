var FALLBACK_DATA = FALLBACK["gold-crown"];
var _projectId = null;
...
function applyData(data) {
  ...
  if (el('reception-date-short')) el('reception-date-short').innerHTML = days[recep.getDay()] + ', ' + recep.getDate() + ' ' + months[recep.getMonth()] + ' ' + recep.getFullYear();

  if (el('akad-venue')) el('akad-venue').innerHTML = (d.akadVenue || f.akadVenue) + (d.akadAddress ? ' — ' + (d.akadAddress || f.akadAddress) : '');
  if (el('reception-venue')) el('reception-venue').innerHTML = (d.receptionVenue || f.receptionVenue) + (d.receptionAddress ? ' — ' + (d.receptionAddress || f.receptionAddress) : '');
...
function renderPayment(paymentData) {
  ...
  list.innerHTML = paymentData.map(function (p) {
    return '<div class="payment-card">'
      + '<img src="' + getPaymentImage(p.method) + '" alt="' + p.method + '" onerror="this.style.display=\'none\'">'
...
function renderGallery(galleryData) {
  ...
  var preview = el('glide-gallery-preview');
  if (!slides || !galleryData.length) return;

  slides.innerHTML = galleryData.map(function (src) {
    return '<li class="glide__slide glide-gallery-slide" data-src="' + src + '"><img src="' + src + '" alt=""></li>';
  }).join('');
...
function renderWishes(wishesData) {
  ...
  if (!wishesData || !wishesData.length) {
    list.innerHTML = '<p class="text-light text-center">Belum ada ucapan</p>';
    return;
  }

  list.innerHTML = wishesData.map(function (w) {
    return '<div class="wish-item">'
      + '<h6>' + sanitize(w.guest_name || w.name) + '</h6>'
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    (function (btn, idx) {
      btn.addEventListener('click', function () {
        for (var j = 0; j < fabs.length; j++) {
          fabs[j].classList.remove('active');
        }
  ...
  if (carousel) {
    ...
    carousel.addEventListener('slid.bs.carousel', function (event) {
      var idx = Array.prototype.indexOf.call(event.target.querySelectorAll('.carousel-item'), event.relatedTarget);
      var fabsX = document.querySelectorAll('.fab-btn');
      for (var j = 0; j < fabsX.length; j++) {
        fabsX[j].classList.toggle('active', j === idx);
      }

      if (event.relatedTarget.querySelector('#section-3') !== null && glideGallery === null) {
        glideGallery = new Glide('.glide-gallery', {
          type: 'carousel',
          perView: 3,
          focusAt: 0,
          gap: 20,
          ...
          var active = document.querySelector('.glide-gallery-slide.glide__slide--active');
          var preview = el('glide-gallery-preview');
          if (active && preview) {
            var src = active.getAttribute('data-src') || active.querySelector('img').src;
            preview.src = src;
          }
        }).mount();
      }
