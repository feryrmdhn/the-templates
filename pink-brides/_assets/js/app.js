var FALLBACK_DATA = FALLBACK["pink-brides"];
var _projectId = null;
var _apiBaseUrl = null;

function el(id) { return document.getElementById(id); }
function elAll(id) { return document.querySelectorAll('#' + id); }
function getPaymentImage(method) { return '/payment/' + method + '.png'; }

function renderTenant(data) {
    data = data || {};

    var logoEl = el('tenant-logo');
    if (logoEl) {
        logoEl.src = data.logo_url || '/placeholder-image.png';
        logoEl.onerror = function () { this.onerror = null; this.src = '/placeholder-image.png'; };
    }

    var socialContainers = elAll('tenant-social');
    var fb = data.facebook_url || '#';
    var ig = data.instagram_url || '#';
    socialContainers.forEach(function (container) {
        container.innerHTML = '<a href="' + fb + '" target="_blank" rel="noopener noreferrer" class="tenant-social-link" aria-label="Facebook"><i class="fa-brands fa-facebook-f"></i></a>'
            + '<a href="' + ig + '" target="_blank" rel="noopener noreferrer" class="tenant-social-link" aria-label="Instagram"><i class="fa-brands fa-instagram"></i></a>';
    });
}
...
function applyData(data) {
    ...
    var coupleText = (d.groomFullName || f.groomFullName) + ' & ' + (d.brideFullName || f.brideFullName);
...
window.addEventListener('message', function (e) {
    ...
    if (payload.mode === "preview") {
        ...
        if (apiBaseUrl && tenantSlug) {
            getTenantLogo(apiBaseUrl, tenantSlug).then(function (res) {
                renderTenant((res && res.data) ? res.data : {});
            }).catch(function () { renderTenant({}); });
        } else {
            renderTenant({});
        }
        notifyLoaded();
    ...
    Promise.all([
        ...
        var tenantRes = results[1];

        renderTenant((tenantRes && tenantRes.data) ? tenantRes.data : {});
...
function renderWishes(wishesData) {
    ...
    list.innerHTML = wishesData.map(function (w) {
        return '<div class="row gx-3 gx-md-5 mb-8">'
            + '<div class="col-auto"><div class="w-5rem h-5rem bg-secondary rounded-circle d-flex align-items-center justify-content-center"><h5><i class="fa-solid fa-user text-primary"></i></h5></div></div>'
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  document.addEventListener('contextmenu', function (e) { e.preventDefault(); });
    ...
    setTimeout(function () {
        if (!el('tenant-social').innerHTML.trim()) renderTenant({});
    }, 3000);
    ...
    window.addEventListener('scroll', function () {
        ...
        for (var s of sectionlistener) {
            if (window.scrollY > (s.offsetTop - 100) && window.scrollY < (s.offsetTop + s.offsetHeight)) {
                for (var link of document.querySelectorAll('.navbar .nav-link')) link.classList.remove('active');
                var activeLink = document.querySelector('.navbar .nav-link[href="#' + s.id + '"]');
