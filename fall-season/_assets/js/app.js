var FALLBACK_DATA = FALLBACK["fall-season"];
var _projectId = null;
...
function applyData(data) {
    ...
    if (coverImg) {
        ...
        if (openingSection) {
            openingSection.style.backgroundImage = 'linear-gradient(rgba(62, 39, 35, 0.65), rgba(62, 39, 35, 0.75)), url(\'' + coverImg + '\')';
        }
        var headerSection = el('header');
        if (headerSection) {
            headerSection.style.backgroundImage = 'linear-gradient(rgba(62, 39, 35, 0.55), rgba(62, 39, 35, 0.7)), url(\'' + coverImg + '\')';
        }
    }
...
function renderStories(stories) {
    ...
    container.innerHTML = stories.map(function (s, i) {
        ...
        if (i < stories.length - 1) {
            html += '<div class="story-divider" data-aos="zoom-in"><img src="_assets/img/decoration/divider-leaf.svg" alt=""></div>';
        }
        return html;
    }).join('');
...
function renderWishes(wishesData) {
    ...
    list.innerHTML = wishesData.map(function (w) {
        ...
        return '<div class="col-md-6" data-aos="fade-up"><div class="wish-card">'
            + '<div class="wish-head"><img src="_assets/img/icons/avatar.svg" class="wish-avatar" alt="">'
...
function showNotification(message, type) {
    ...
    notification.style.cssText = 'position:fixed;top:20px;left:50%;transform:translateX(-50%);z-index:99999;'
        + 'background:' + (type === 'success' ? '#1B5E20' : '#B22222') + ';color:white;'
        + 'padding:16px 24px;border-radius:8px;box-shadow:0 4px 12px rgba(62,39,35,0.15);'
        + 'font-size:14px;font-weight:500;font-family:Lato,sans-serif;max-width:90%;text-align:center;'
...
function startLeaves() {
    var container = document.querySelector('.falling-leaves-container');
    if (!container) return;
    container.innerHTML = '';

    var leafSvgs = [
        '_assets/img/decoration/leaf-maple-1.svg',
        '_assets/img/decoration/leaf-maple-2.svg',
        '_assets/img/decoration/leaf-maple-3.svg',
        '_assets/img/decoration/leaf-oak-1.svg',
        '_assets/img/decoration/leaf-oak-2.svg'
    ];
    var leafCount = window.innerWidth < 576 ? 8 : 15;

    for (var i = 0; i < leafCount; i++) {
        var leaf = document.createElement('img');
        leaf.className = 'falling-leaf';
        leaf.src = leafSvgs[Math.floor(Math.random() * leafSvgs.length)];
        leaf.style.left = (Math.random() * 100) + '%';
        leaf.style.width = (2 + Math.random() * 2) + 'rem';
        leaf.style.animationDuration = (8 + Math.random() * 7) + 's';
        leaf.style.animationDelay = (Math.random() * 10) + 's';
        leaf.style.opacity = String(0.6 + Math.random() * 0.4);
        leaf.classList.add('fall-sway-' + (Math.floor(Math.random() * 3) + 1));
