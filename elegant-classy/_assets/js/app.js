var FALLBACK_DATA = FALLBACK["elegant-classy"];
var _projectId = null;
...
function applyData(data) {
    ...
    var coupleShort = (d.groomName || f.groomName) + ' & ' + (d.brideName || f.brideName);
    if (el('navbar-couple')) el('navbar-couple').innerHTML = coupleShort;
    if (el('header-couple')) el('header-couple').innerHTML = (d.groomFullName || f.groomFullName) + '<br>&<br>' + (d.brideFullName || f.brideFullName);
    ...
    if (el('section5-quote')) el('section5-quote').innerHTML = d.section5Quote || f.section5Quote;
    if (el('section5-quote-source')) el('section5-quote-source').innerHTML = d.section5QuoteSource || f.section5QuoteSource;

    var openingEl = document.querySelector('section#opening');
    if (openingEl && (d.coverImage || f.coverImage)) {
        openingEl.style.backgroundImage = 'linear-gradient(180deg, rgba(0, 0, 0, 0.52) 0%, rgba(0, 0, 0, 0.52) 100%), url(' + (d.coverImage || f.coverImage) + ')';
    }

    var headerEl = document.querySelector('header#header');
    if (headerEl && (d.coverImage || f.coverImage)) {
        headerEl.style.backgroundImage = 'linear-gradient(180deg, rgba(0, 0, 0, 0.52) 0%, rgba(0, 0, 0, 0.52) 100%), url(' + (d.coverImage || f.coverImage) + ')';
    }

    var section5El = document.querySelector('section#section-5');
    if (section5El && (d.backgroundSource || f.backgroundSource)) {
        section5El.style.backgroundImage = 'linear-gradient(180deg, rgba(0, 0, 0, 0.52) 0%, rgba(0, 0, 0, 0.52) 100%), url(' + (d.backgroundSource || f.backgroundSource) + ')';
    }
    ...
    if (el('akad-maps-frame')) el('akad-maps-frame').src = cleanMapsUrl(d.akadMapsUrl || f.akadMapsUrl);
    ...
    if (el('reception-maps-frame')) el('reception-maps-frame').src = cleanMapsUrl(d.receptionMapsUrl || f.receptionMapsUrl);
    ...
    if (el('header-date')) el('header-date').innerHTML = days[recep.getDay()] + ', ' + recep.getDate() + ' ' + months[recep.getMonth()] + ' ' + recep.getFullYear();
    ...
    if (isShowStory) {
        ...
        var storySection = document.querySelector('section#section-2');
        if (storySection) storySection.style.display = 'none';
        var storyNav = document.querySelector('.nav-link[href="#section-2"]');
    ...
    if (isShowGallery) {
        ...
        var gallerySection = document.querySelector('section#section-3');
        if (gallerySection) gallerySection.style.display = 'none';
        var galleryNav = document.querySelector('.nav-link[href="#section-3"]');
...
function animateLettersWithBreaks(selector, baseDelay) {
...
function renderStory(storyItems) {
    ...
    var leftItems = storyItems.filter(function (_, i) { return i % 2 === 0; });
    var rightItems = storyItems.filter(function (_, i) { return i % 2 !== 0; });

    var makeCard = function (s, dir) {
        return '<div class="card card-story card-story-' + dir + ' mx-auto mb-6 mb-sm-10 mb-lg-16 story-animate story-animate-' + dir + '">'
            + '<div class="card-body"><img src="' + (s.image || '') + '" alt=""></div></div>'
            + '<div class="text-center text-lg-start mb-16 mb-lg-50 story-animate story-animate-up">'
            + '<h5>' + (s.year || '') + '</h5>'
            + '<h4 class="font-type-secondary mb-2">' + s.title + '</h4>'
            + '<p class="text-lg font-light">' + s.description + '</p>'
            + '</div>';
    };

    var leftHtml = leftItems.map(function (s) { return makeCard(s, 'left'); }).join('');
    var rightHtml = rightItems.map(function (s) { return makeCard(s, 'right'); }).join('');

    list.innerHTML = '<div class="col-lg-5 mb-6 mb-sm-10 mb-lg-0">' + leftHtml + '</div>'
        + '<div class="d-none d-lg-block col-lg-auto text-center"><img src="_assets/img/decoration/list-section-2.svg" alt=""></div>'
        + '<div class="col-lg-5">' + rightHtml + '</div>';

    var lastScrollY = window.scrollY;

    var observer = new IntersectionObserver(function (entries) {
        var goingDown = window.scrollY >= lastScrollY;
    ...
    list.querySelectorAll('.story-animate').forEach(function (el2) {
...
function renderGallery(galleryData) {
    ...
    items.forEach(function (id, i) {
        ...
        if (galleryData[i]) {
            ...
            el2.innerHTML = '<img src="' + galleryData[i] + '" alt="" style="width:100%;height:100%;object-fit:cover;">';
        } else {
...
function renderWishes(wishesData) {
    ...
    if (!wishesData || !wishesData.length) {
        list.innerHTML = '<li class="li list-group-item text-center py-4">'
            + '<p class="text-lg font-light">Belum ada ucapan</p>'
            + '</li>';
        return;
    }
...
function renderPayment(paymentData) {
    ...
    list.innerHTML = paymentData.map(function (p) {
        return '<div class="text-center p-3" style="min-width:16rem; display:flex; flex-direction:column; justify-content:center; align-items:center;">'
...
function updateCountdown(datetime) {
    ...
    var countdownEl = el('countdown-elegant');
    ...
    _countdownInstance = timezz(countdownEl, {
        ...
        update: function (data) {
            var days = countdownEl.querySelector('[data-days]');
            var hours = countdownEl.querySelector('[data-hours]');
            var minutes = countdownEl.querySelector('[data-minutes]');
            var seconds = countdownEl.querySelector('[data-seconds]');
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (btn_open) {
        btn_open.addEventListener('click', function () {
            ...
            setTimeout(function () {
                ...
                AOS.refresh();
                if (el('header-couple')) {
                    animateLettersWithBreaks('#header-couple');
                }
            }, 2000);
    ...
    window.addEventListener('scroll', function () {
        ...
        for (var s of sectionlistener) {
            if (window.scrollY > (s.offsetTop - 150) && window.scrollY < (s.offsetTop + s.offsetHeight)) {
