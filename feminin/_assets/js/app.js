var FALLBACK_DATA = FALLBACK["feminin"];

var _sliderGallery = null;
var _isDataApplied = false; 
var _currentAudioSrc = null; 
var _projectId = null; 
...
function applyData(data) {
    ...
    if (el('section1-photo')) el('section1-photo').src = d.section1Photo || f.section1Photo;
    ...
    function initSlider() {
        try {
            _sliderGallery = tns({
                ...
                responsive: {
                    568: { items: 1, gutter: 20, edgePadding: 60 },
                    992: { items: 1, gutter: 30, edgePadding: 200 }
                }
            });
...
function renderWishes(wishesData) {
    ...
    if (!wishesData || !wishesData.length) {
        list.innerHTML = '<li class="list-group-item text-center"><p class="text-lg font-light mb-0">Belum ada ucapan</p></li>';
        return;
    }
    ...
    list.innerHTML = wishesData.map(function (w) {
        return '<li class="li list-group-item">'
            ...
            + '<p class="text-lg font-light mb-1">' + sanitize(w.message) + '</p>'
            + '</li>';
    }).join('');
}
...
function handleCommentSubmit(e) {
    ...
    postComment(_apiBaseUrl, payload)
        .then(function (response) {
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  document.addEventListener('contextmenu', function (e) { e.preventDefault(); });
    ...
    var rsvpForm = document.querySelector('#section-2 form');
    if (rsvpForm) {
